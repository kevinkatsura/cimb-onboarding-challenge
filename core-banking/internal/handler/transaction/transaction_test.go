...
import (
	...
	transaction "core-banking/internal/handler/transaction"
	"core-banking/mocks"
	"errors"
	"net/http"
	"net/http/httptest"
...
func TestTransactionHandler_Transfer(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mockSetup: func(m *MockerForHandler) {
				resp := &dto.TransferResponse{Status: "success"}
				m.service.On("Transfer", mock.Anything, mock.Anything).
					Return(resp, nil)
			},
			wantErr: false,
...
// 			},
// 			mockSetup: func(m *MockerForHandler) {
// 				m.service.On("List", mock.Anything, mock.MatchedBy(func(f domain.TransactionListFilter) bool {
// 					return f.Limit == 10 &&
// 						f.Type != nil &&
// 						f.Status != nil &&
// 						*f.Type == "debit" &&
