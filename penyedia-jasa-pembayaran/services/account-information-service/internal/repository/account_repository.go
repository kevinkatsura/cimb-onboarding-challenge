...
func (db *PostgresDatabase) UpsertAccount(ctx context.Context, acc Account) error {
	query := `
		INSERT INTO accounts (account_number, account_id, customer_id, product_code, currency, status, balance, created_at, updated_at)
...
func (db *PostgresDatabase) UpsertTransaction(ctx context.Context, tx Transaction) error {
	query := `
		INSERT INTO transactions (transaction_ref, source_account_number, beneficiary_account_number, amount, currency, created_at)
		VALUES ($1, $2, $3, $4, $5, $6)
...
func (db *PostgresDatabase) GetBalance(ctx context.Context, accountNo string) (int64, string, error) {
	var balance int64
	var currency string
	err := db.Pool.QueryRow(ctx, "SELECT balance, currency FROM accounts WHERE account_number = $1", accountNo).Scan(&balance, &currency)
	return balance, currency, err
}

func (db *PostgresDatabase) GetAccountInfo(ctx context.Context, accountNo string) (Account, error) {
	var acc Account
	err := db.Pool.QueryRow(ctx, "SELECT account_number, account_id, customer_id, product_code, currency, status, balance, created_at, updated_at FROM accounts WHERE account_number = $1", accountNo).
...
func (db *PostgresDatabase) GetLastTransactionAsSource(ctx context.Context, accountNo string) (Transaction, error) {
	var tx Transaction
	err := db.Pool.QueryRow(ctx, "SELECT transaction_ref, source_account_number, beneficiary_account_number, amount, currency, created_at FROM transactions WHERE source_account_number = $1 ORDER BY created_at DESC LIMIT 1", accountNo).
...
func (db *PostgresDatabase) GetLastTransactionAsBeneficiary(ctx context.Context, accountNo string) (Transaction, error) {
	var tx Transaction
	err := db.Pool.QueryRow(ctx, "SELECT transaction_ref, source_account_number, beneficiary_account_number, amount, currency, created_at FROM transactions WHERE beneficiary_account_number = $1 ORDER BY created_at DESC LIMIT 1", accountNo).
		Scan(&tx.TransactionRef, &tx.SourceAccountNumber, &tx.BeneficiaryAccountNumber, &tx.Amount, &tx.Currency, &tx.CreatedAt)
	return tx, err
}
